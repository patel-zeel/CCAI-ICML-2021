...
inds, _ = greedy.place(trn_X, N=n, method='Var')
best_model = None
best_loss = np.inf
for i in range(2,9):
    try:
        model = LLS(trn_X.shape[1], N_l_bar=i, N_l_bar_method='greedy')
        model.fit(trn_X, trn_y-mean_y, n_restarts=1, near_opt_inds=inds[:i])
